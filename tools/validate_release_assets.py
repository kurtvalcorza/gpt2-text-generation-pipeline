...
EXPECTED_OUTPUTS = ('outputs/gpt2_text_generation_result.json',)
# Profile-specific code the notebook must exercise through the repository public API.
CODE_MARKERS = (
    'from gpt2_text_generation_pipeline import CONTEXT_LENGTH, DEFAULT_MAX_NEW_TOKENS, EOS_TOKEN_ID, '
    'MAX_NEW_TOKENS, MAX_PROMPT_TOKENS, MAX_TEXT_CHARS, PAD_TOKEN_ID, VOCAB_SIZE, validate_settings',
    'from gpt2_text_generation_pipeline import MODEL_ID, MODEL_KEY, MODEL_REVISION, '
    'GPT2TextGenerationPipeline, stage_missing_files, verify_snapshot',
    ...
    'sampled = pipe.generate(prompt, max_new_tokens=SAMPLE_MAX_NEW_TOKENS, do_sample=True, '
    'temperature=TEMPERATURE, top_p=TOP_P, seed=SEED)',
    "ceilings = {'CONTEXT_LENGTH': CONTEXT_LENGTH, 'MAX_PROMPT_TOKENS': MAX_PROMPT_TOKENS, "
    "'MAX_NEW_TOKENS': MAX_NEW_TOKENS, 'DEFAULT_MAX_NEW_TOKENS': DEFAULT_MAX_NEW_TOKENS, "
    "'MAX_TEXT_CHARS': MAX_TEXT_CHARS, 'VOCAB_SIZE': VOCAB_SIZE, 'EOS_TOKEN_ID': EOS_TOKEN_ID, "
...
MARKDOWN_MARKERS = (
    '**Capability:** causal text generation (continuing one English prompt)',
    '**Two decoding modes are demonstrated and must not be confused (INF8):**',
    '**No adaptation occurs:**',
    '**base language model**',
    '**No quality metric exists**',
    '**no metric helper and reports no performance measure**',
    '**no metric is reported**',
    '**Greedy decoding is deterministic on a fixed device and dtype:**',
    '**mandatory `seed`**',
    '**GPT-2 ships no pad token, so the pipeline fixes `pad_token_id = eos_token_id = 50256` in code**',
    '`MAX_PROMPT_TOKENS`',
